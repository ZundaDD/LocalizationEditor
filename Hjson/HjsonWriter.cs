...
internal class HjsonWriter
{
    ...
    static readonly Regex needsEscapeName = new(@"[,\{\[\}\]\s:#""']|\/\/|\/\*|'''");
    ...
    public void Save(JsonValue value, TextWriter tw, int level, bool hasComment, string separator, bool noIndent = false, bool isRootObject = false)
    {
        ...
        switch (value.JsonType)
        {
            case JsonType.Object:
                ...
                if (!noIndent) { if (obj.Count > 0) tw.Write(' '); else tw.Write(separator); }
    ...
    static bool needsQuotes(char c) => c is '\t' or '\f' or '\b' or '\n' or '\r';

    static bool needsEscape(char c) => c is '\"' or '\\' || needsQuotes(c);

    static bool needsEscapeML(char c) => c is '\f' or '\b';
}